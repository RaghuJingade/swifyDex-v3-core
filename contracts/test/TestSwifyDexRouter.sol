...
contract TestSwifyDexRouter is ISwifyDexSwapCallback {
    ...
    function swifyDexSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? ISwifyDexPool(msg.sender).token0()
                : ISwifyDexPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0
                ? amount0Delta
                : amount1Delta;

            bool zeroForOne = tokenToBePaid == ISwifyDexPool(pools[0]).token1();
            ISwifyDexPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
                zeroForOne
